...
//
// tofFrameReady is set by handleReceivedResultDataEnd() and is never cleared
// here, and tofFrame.complete latches true once the first frame lands. A
// consumer that needs a *new* frame rather than the latest one must clear
// tofFrameReady itself before each wait, or it will read one stale frame N
// times over. See TofSensor::measure().
#define TOF_ZONES 64
